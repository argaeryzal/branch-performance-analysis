...
# Top 5 Cabang dengan Pendapatan Paling Besar
df_loan_mei %>% 
  arrange(desc(total_amount)) %>% 
  mutate(total_amount = comma(total_amount)) %>% 
  head(5)

# Top 5 Cabang dengan Pendapatan Kecil
df_loan_mei %>%
...
# Gabung df_loan_mei dengan df_cabang_umur
df_loan_mei_umur <- df_cabang_umur %>%
  inner_join(df_loan_mei, by = 'cabang')
df_loan_mei_umur

# Visualisasi untuk melihat relasi umur cabang dengan performa bulan mei
...
# Mencari cabang yang memiliki performa rendah
...
# Perbadingan performa agen pada cabang yang rendah
df_loan_mei_flag %>% 
...
# Perbadingan performa agen pada cabang yang paling baik umur 3 bulan
df_loan %>% 
  filter(cabang == 'AH') %>% 
  filter(tanggal_cair >= '2020-05-01', tanggal_cair <= '2020-05-31') %>% 
  group_by(cabang, agen) %>% 
